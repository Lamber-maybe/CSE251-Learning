...
int main(){
    ...
    while(true){
        ...
        /* Get the command */
        printf("Command Help:\n");
        printf("1. quit -- Exit the game           \n");
        printf("2. move -- Move forward            \n");
        printf("3. turn -- Change direction        \n");
        printf("4. fire -- Fire within three steps \n");
        printf("Command: ");
        scanf("%20s", command);
        if(strcmp(command, "quit") == 0) {
            ...
            printf("fire!...");
            d = DifferenceByDirection(agentDirection);
            for(int j=1; j <=3 && *(agentRoom + d * j) != End; j++){
                if(*(agentRoom + d * j) == Wumpus){
                    *(agentRoom + d * j) = Empty;
                }
            }
        }
        else{
            printf("I don't known what you are talking about\n");
        }
    }
}
...
bool DisplayStatus(int cave[ArraySize][ArraySize], int *agent){
    ...
    if(!find){
        printf("You kill Wumpus, you WIN!!!\n");
        return true;
    }
